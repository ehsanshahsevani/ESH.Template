...
public class CheckCategoryIdActionFilter : ESH.SeedworkSystem.Infrastructure.ActionFilter.IBaseAsyncActionFilter
{
	...
	public async Task OnActionExecutionAsync(ActionExecutingContext context, ActionExecutionDelegate next)
	{
		...
		else
		{
			var category =
				await UnitOfWork
					.CategoryRepository.FindAdminAsync(id: id);

			if (category is null)
			{
				var errorMessage = string.Format(
					format: ESH.Resources.Messages.NotFoundError, arg0: ESH.Resources.DataDictionary.Category);

				result.WithError(errorMessage: errorMessage);
			}
			else
			{
				context.HttpContext.Items[key: ProjectKeyName.ObjectKey] = category;
			}
		}
